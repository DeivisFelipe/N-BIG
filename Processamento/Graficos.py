...
def main():
    ...
    print("Maior duração: ", bigger_duration, flush=True)
    print("Menor duração: ", smaller_duration, flush=True)
    print("Maior quantidade de bytes: ", bigger_bytes, flush=True)
    print("Menor quantidade de bytes: ", smaller_bytes, flush=True)
...
def duration_histogram(collection, duration_intervals, flows_by_duration_counters, packets_by_duration_counters, total_bytes_by_duration_counters):
    print("*" * 50, flush=True)
    print("Gerando histograma de duração dos fluxos...", flush=True)
    for i in range(NUMBER_BINS_HISTOGRAMA):
    ...
    print("Quantidade de fluxos por duracao gerado com sucesso!", flush=True)

def bytes_histogram(collection, bytes_intervals, flows_by_bytes_counters):
    print("*" * 50, flush=True)
    print("Gerando histograma de bytes dos fluxos...", flush=True)
    for i in range(NUMBER_BINS_HISTOGRAMA):
        if i == NUMBER_BINS_HISTOGRAMA - 1:
    ...
    print("Quantidade de fluxos por bytes gerado com sucesso!", flush=True)
...
def average_packet_size_by_duration_histogram(duration_intervals, packets_by_duration_counters, total_bytes_by_duration_counters):
    print("*" * 50, flush=True)
    print("Gerando histograma de tamanho médio dos pacotes por duração...", flush=True)
    tamanho_medio = []
    for i in range(NUMBER_BINS_HISTOGRAMA):
    ...
    print("Histograma de tamanho médio dos pacotes por duração gerado com sucesso!", flush=True)

if __name__ == '__main__':
    start_time = time.time()
    print("Iniciando a geração dos graficos...", flush=True)
    print("Horário:", time.strftime("%H:%M:%S", time.localtime(start_time)), flush=True)
    print("=" * 50, flush=True)
    main()
    print("=" * 50, flush=True)
    end_time = time.time()
    execution_time = end_time - start_time
    print("Tempo de execução: ", execution_time, " segundos", flush=True)